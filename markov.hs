...
type Link a   = (a, Int)
--data MChain a = MarkovChain a( M.Map a [Link a] ) deriving (Show)
type Words    = [String]
...
main = do
  ...
  let mchain = chainWords . map (filter (\x -> ord x > 32 && ord x < 126 && ord x /= 34)) . words $ input
  -- 
  ...
  print sen1
  -- sen2 <- randomSentence "president" mchain
  -- sen3 <- randomSentence "love" mchain
